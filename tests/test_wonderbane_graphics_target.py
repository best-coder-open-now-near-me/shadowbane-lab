...
def test_graphics_publication_and_launch_pin_the_golden_package() -> None:
    ...
    assert 'ExtensionVersion = "1.6.8"' in publish
    assert "wonderbane-extension.dll" in publish
    assert r"\build\wonderbane-client-extension\Release" in publish
    assert "9a6a6e8b90c96d8be37e8bf4add906b979c22363be1542cac104aa7fa0a2b4d4" in publish
    ...
    assert 'ExtensionVersion = "1.6.8"' in launch
    assert "9a6a6e8b90c96d8be37e8bf4add906b979c22363be1542cac104aa7fa0a2b4d4" in launch
    ...
    assert "std::array<ImportHookPlan, 19U>" in cel_shading
...
def test_extension_version_is_consistent_across_every_runtime_surface() -> None:
    ...
    assert "project(wonderbane_extension VERSION 1.6.8" in cmake
    assert 'kExtensionVersion[] = "1.6.8"' in extension
    assert 'kExtensionVersion[] = "1.6.8"' in graphics_status
    assert "WONDERBANE_EXTENSION_VERSION_MAJOR 1U" in api
    assert "WONDERBANE_EXTENSION_VERSION_MINOR 6U" in api
    assert "WONDERBANE_EXTENSION_VERSION_PATCH 8U" in api
    assert "FILEVERSION 1,6,8,0" in resource
    assert "PRODUCTVERSION 1,6,8,0" in resource
    assert 'VALUE "FileVersion", "1.6.8.0\\0"' in resource
    assert 'VALUE "ProductVersion", "1.6.8.0\\0"' in resource
