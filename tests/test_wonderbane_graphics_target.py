...
def test_extension_version_is_consistent_across_every_runtime_surface() -> None:
    ...
    assert "project(wonderbane_extension VERSION 1.7.1" in cmake
    assert 'kExtensionVersion[] = "1.7.1"' in extension
    assert 'kExtensionVersion[] = "1.7.1"' in graphics_status
    ...
    assert "FILEVERSION 1,7,1,0" in resource
    assert "PRODUCTVERSION 1,7,1,0" in resource
    assert 'VALUE "FileVersion", "1.7.1.0\\0"' in resource
    assert 'VALUE "ProductVersion", "1.7.1.0\\0"' in resource
