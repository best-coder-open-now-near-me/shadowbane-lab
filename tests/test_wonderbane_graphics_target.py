...
def test_graphics_publication_and_launch_pin_the_golden_package() -> None:
    ...
    assert 'ExtensionVersion = "1.6.7"' in publish
    assert "wonderbane-extension.dll" in publish
    assert r"\build\wonderbane-client-extension\Release" in publish
    assert "96881f786ddb4c7915847983ddc1770c21decc491ffa77a2350562cbdd284b4c" in publish
    ...
    assert 'ExtensionVersion = "1.6.7"' in launch
    assert "96881f786ddb4c7915847983ddc1770c21decc491ffa77a2350562cbdd284b4c" in launch
...
def test_extension_version_is_consistent_across_every_runtime_surface() -> None:
    ...
    assert "project(wonderbane_extension VERSION 1.6.7" in cmake
    assert 'kExtensionVersion[] = "1.6.7"' in extension
    assert 'kExtensionVersion[] = "1.6.7"' in graphics_status
    assert "WONDERBANE_EXTENSION_VERSION_MAJOR 1U" in api
    assert "WONDERBANE_EXTENSION_VERSION_MINOR 6U" in api
    assert "WONDERBANE_EXTENSION_VERSION_PATCH 7U" in api
    assert "FILEVERSION 1,6,7,0" in resource
    assert "PRODUCTVERSION 1,6,7,0" in resource
    assert 'VALUE "FileVersion", "1.6.7.0\\0"' in resource
    assert 'VALUE "ProductVersion", "1.6.7.0\\0"' in resource
