...
The open world map is also a native destination input: left-click a zone emblem or any
other valid point on the map to start the same closed-loop `/go` route; right-click remains
supported. The listener reads `ArcWorldMapHud`'s live rectangle, hidden state, world dimensions,
