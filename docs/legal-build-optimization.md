...
The integrated strict evaluator runs compiled candidates over common-seed, mirrored scenarios
using the reference simulator and keeps strict and candidate archive admission separate.
[Legal-build training](legal-build-training.md) documents calculator-approved mutation,
provenance-bound training-budget gates, and the current Irekei Assassin experiment.
[Utility-policy training](utility-policy-training.md) owns policy search separately from build
selection. Full-covariance CMA-ES and an optimized NumPy/Numba backend remain later work; a faster
backend must match reference traces before replacing the oracle.
