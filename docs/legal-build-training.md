...
- the typed, provenance-bound training-budget gate rejects proven overspending
  against a supported identity's sourced pool, using conservative rank-cost
  lower bounds and preserving unsupported identities as unresolved.

The training check is necessary but not sufficient. Exact general skill/power
rank costs and opaque item requirement tokens remain unresolved, so passing the
gate never promotes a candidate to strict evidence. The generic compiler also
retains its explicit training-budget coverage limitation.
...
1. extend the typed training-budget profiles with sourced exact skill/power costs
   and coverage for currently unsupported identities;
