...
Integrated extension 1.6.10 adds an optional screen-space filter for one-pixel depth cracks. It is
an experimental contour policy, not a terrain repair. It retains the exact reviewed main-clear
and done3D world/UI boundary recovered in 1.6.9, not the superseded 1.6.8 heuristic.
...
The combined 1.6.10 publication and launch scripts pin a newly built artifact; previous release
artifacts remain unchanged. Deploy its schema-2 Graphics Lab and DLL together. Local build/probe
validation is not live acceptance: visual comparison, restart verification, and frame-cost
measurement on the testing VM remain required before a player-facing release.
