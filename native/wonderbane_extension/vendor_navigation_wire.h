...
constexpr std::uint32_t magic = 0x57424e33, ready = 1, in_flight = 2, unresolved = 4;
#pragma pack(push, 1)
struct Snapshot {
    std::uint64_t scene = 0, revision = 0;
    std::uint32_t root = 0, manager = 0, front_hud = 0, mode = 0;
...
inline bool OwnsBuilding(const Snapshot& s, Key building) noexcept {
    return !s.offline && s.building == building && (s.visible & 1);
}
inline bool Opened(const Snapshot& s, Verb verb, Key building, Key vendor = {}) noexcept {
    if (s.offline || s.building != building || !s.front_hud) { return false; }
    if (verb == Verb::warehouse) {
        return Typed(vendor, 42) && s.warehouse_hud && s.warehouse_object
            && s.warehouse == vendor && s.front_hud == s.warehouse_hud;
    }
    return verb == Verb::building
        ? OwnsBuilding(s, building) && s.front_hud == s.building_hud
        : Matches(verb, vendor) && (s.visible & 2) && s.vendor == vendor
            && s.front_hud == s.vendor_hud;
}
inline bool Valid(Verb verb, const Command& c) noexcept {
    ...
    return (verb == Verb::building && c.vendor == Key{})
        || ((Matches(verb, c.vendor) || (verb == Verb::warehouse && Typed(c.vendor, 42)))
            && OwnsBuilding(c.expected, c.building));
}
}
