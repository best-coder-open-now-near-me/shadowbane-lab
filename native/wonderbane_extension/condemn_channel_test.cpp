...
namespace v = wonderbane::extension::condemn;
...
int main() {
    ...
    auto publish = [&](std::uint64_t sequence, bool wrong_lease, unsigned kind = 23) {
        ...
        if (kind != 23) {
            ...
            s.scene = s.revision = 1; s.local = {99, 53}; s.root = 100; s.manager = 200;
            s.building_hud = s.front = 300; s.open_button = 400; s.building = {123, 8};
            slot.kind = 24;
        }
        payload.target = {{123, 8}, {777, 23}, kind == 25 ? 3U : 5U};
    ...
    assert(runtime.condemn_pending && !runtime.pending && storage.header.command_read_sequence == 0);
    ...
    assert(!runtime.condemn_pending && storage.header.command_read_sequence == 1);
    ...
    assert(d::DrainCommands(storage, runtime.result_signal, now) == ERROR_IO_PENDING);
    command = v::Take();
    assert(command && command->verb == v::wire::Verb::ensure && command->command.target.scope == 5);
    ...
    publish(5, false, 25); // An unsupported scope never reaches the owning thread.
    assert(d::DrainCommands(storage, runtime.result_signal, now) == ERROR_SUCCESS);
