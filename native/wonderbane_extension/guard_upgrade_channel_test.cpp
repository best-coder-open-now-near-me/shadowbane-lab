...
namespace v = wonderbane::extension::guard_upgrade;
...
int main() {
    ...
    auto publish = [&](std::uint64_t sequence, bool wrong_lease, unsigned kind = 17) {
        ...
        if (kind != 17) {
            auto& s = payload.expected.navigation;
            s.scene = s.revision = 1; s.root = 100; s.manager = 200; s.mode = 6;
            s.building_hud = 300; s.vendor_hud = 400; s.selected_entry = 500;
            s.visible = 3; s.initialized = 1; s.building = {123, 8}; s.vendor = {777, 37};
            payload.expected.rank = 1; payload.expected.cost = 100;
            payload.expected.funds = kind == 19 ? 99 : 150;
            payload.expected.can_upgrade = 1; payload.expected.control_flags = 3;
            payload.expected.upgrade_control = 600; payload.expected.progress_control = 700;
            slot.kind = 18;
        }
    ...
    assert(runtime.guard_upgrade_pending && !runtime.pending && storage.header.command_read_sequence == 0);
    ...
    assert(!runtime.guard_upgrade_pending && storage.header.command_read_sequence == 1);
    ...
    assert(d::DrainCommands(storage, runtime.result_signal, now) == ERROR_IO_PENDING);
    command = v::Take();
    assert(command && command->verb == v::wire::Verb::upgrade && command->command.expected.navigation.vendor[1] == 37);
    ...
    publish(5, false, 19); // An underfunded upgrade never reaches the owning thread.
    assert(d::DrainCommands(storage, runtime.result_signal, now) == ERROR_SUCCESS);
