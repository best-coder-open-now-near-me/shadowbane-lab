...
FrameError ValidateFrame(const void* const data, const std::size_t size,
    ...
    if (frame.zone_id == 0U || frame.zone_id != frame.live_zone_id) {
        return FrameError::zone;
    }
    ...
    if (frame.lease_ms > now_ms || now_ms - frame.lease_ms > 2000U) {
        return FrameError::lease;
    }
    if (!FrameLeaseValid(frame, now_ms)) {
        return FrameError::stale;
    }
