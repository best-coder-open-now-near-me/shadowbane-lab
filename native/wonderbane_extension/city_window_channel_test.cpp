...
namespace v = wonderbane::extension::city_window;
...
int main() {
    ...
    auto publish = [&](std::uint64_t sequence, bool wrong_lease) {
        ...
        slot = {};
        slot.command_id = sequence; slot.kind = 11; slot.payload_version = 1;
    ...
    assert(runtime.city_window_pending && !runtime.pending && storage.header.command_read_sequence == 0);
    ...
    assert(!runtime.city_window_pending && storage.header.command_read_sequence == 1);
