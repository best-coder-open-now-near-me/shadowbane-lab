...
bool live = true, setter_ok = true, invalidate_purse = false;
std::uint32_t base = 0, purse = 500, confirmations = 0, admissions = 0;
...
bool __fastcall Withdraw(void* self, void*, std::uint32_t event, std::uint32_t flag) {
    assert(reinterpret_cast<std::uint32_t>(self) == base + 0x8000 && !event && !flag);
...
int main() {
    ...
    Vector(hud + 0x54, base + 0x10000, {list});
