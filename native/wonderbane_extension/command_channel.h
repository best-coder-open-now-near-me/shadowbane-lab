...
inline DWORD FinishVendorPending(Runtime& runtime, ULONGLONG now) noexcept {
    const auto& pending = runtime.vendor_pending;
    if (!pending) { return ERROR_SUCCESS; }
    ...
    if (now > pending->deadline) {
        ...
        if (pending->state.compare_exchange_strong(queued, 1)) {
            vendor::wire::Receipt receipt{};
            receipt.host = pending->command.host; receipt.request = pending->command.request;
            receipt.window = pending->command.window;
            receipt.outcome = static_cast<unsigned>(vendor::wire::Outcome::stale);
            vendor::Complete(pending, receipt);
        }
    }
    if (pending->state.load(std::memory_order_acquire) != 2) { return ERROR_IO_PENDING; }
    const auto outcome = static_cast<vendor::wire::Outcome>(pending->receipt.outcome);
    const bool accepted = outcome == vendor::wire::Outcome::observed || outcome == vendor::wire::Outcome::submitted;
    ...
    constexpr char detail[] = "native_vendor_receipt_v1";
    if (!TryPublishResult(*runtime.storage, runtime.result_signal, static_cast<LONG64>(pending->sequence), pending->id,
        accepted ? ClientActionResultStage::submitted_to_client : ClientActionResultStage::rejected_by_client,
        accepted ? ERROR_SUCCESS : ERROR_REQUEST_ABORTED, detail, sizeof(detail) - 1, now,
        &wire_bytes, pending->execution_thread)) { return ERROR_NOT_ENOUGH_QUOTA; }
    InterlockedExchange64(&runtime.storage->header.command_read_sequence, static_cast<LONG64>(pending->sequence));
    vendor::Release(pending); runtime.vendor_pending.reset(); return ERROR_SUCCESS;
}

inline DWORD FinishCityWindowPending(Runtime& runtime, ULONGLONG now) noexcept {
    const auto& pending = runtime.city_window_pending;
    if (!pending) { return ERROR_SUCCESS; }
    // An expired request that never reached the owning thread cannot block the
    // channel forever while minimized/logged out. Never cancel an executing call.
    if (now > pending->deadline) {
        unsigned queued = 0;
        if (pending->state.compare_exchange_strong(queued, 1)) {
            city_window::wire::Receipt receipt{};
            receipt.host = pending->command.host; receipt.request = pending->command.request;
            receipt.window = pending->command.window;
            receipt.outcome = static_cast<unsigned>(city_window::wire::Outcome::stale);
            city_window::Complete(pending, receipt);
        }
    }
    if (pending->state.load(std::memory_order_acquire) != 2) { return ERROR_IO_PENDING; }
    const auto outcome = static_cast<city_window::wire::Outcome>(pending->receipt.outcome);
    const bool accepted = outcome == city_window::wire::Outcome::observed || outcome == city_window::wire::Outcome::submitted;
    movement::wire::Receipt wire_bytes{};
    static_assert(sizeof(wire_bytes) == sizeof(pending->receipt));
    std::memcpy(&wire_bytes, &pending->receipt, sizeof(wire_bytes));
    constexpr char detail[] = "native_city_window_receipt_v1";
    if (!TryPublishResult(*runtime.storage, runtime.result_signal, static_cast<LONG64>(pending->sequence), pending->id,
        accepted ? ClientActionResultStage::submitted_to_client : ClientActionResultStage::rejected_by_client,
        accepted ? ERROR_SUCCESS : ERROR_REQUEST_ABORTED, detail, sizeof(detail) - 1, now,
        &wire_bytes, pending->execution_thread)) { return ERROR_NOT_ENOUGH_QUOTA; }
    InterlockedExchange64(&runtime.storage->header.command_read_sequence, static_cast<LONG64>(pending->sequence));
    city_window::Release(pending); runtime.city_window_pending.reset(); return ERROR_SUCCESS;
}

inline DWORD FinishVendorNavigationPending(Runtime& runtime, ULONGLONG now) noexcept {
    const auto& pending = runtime.vendor_navigation_pending;
    if (!pending) { return ERROR_SUCCESS; }
    // An expired request that never reached the owning thread cannot block the
    // channel forever while minimized/logged out. Never cancel an executing call.
    if (now > pending->deadline) {
        unsigned queued = 0;
        if (pending->state.compare_exchange_strong(queued, 1)) {
            vendor_navigation::wire::Receipt receipt{};
            receipt.host = pending->command.host; receipt.request = pending->command.request;
            receipt.window = pending->command.window;
            receipt.outcome = static_cast<unsigned>(vendor_navigation::wire::Outcome::stale);
            vendor_navigation::Complete(pending, receipt);
        }
    }
    if (pending->state.load(std::memory_order_acquire) != 2) { return ERROR_IO_PENDING; }
    const auto outcome = static_cast<vendor_navigation::wire::Outcome>(pending->receipt.outcome);
    const bool accepted = outcome == vendor_navigation::wire::Outcome::observed || outcome == vendor_navigation::wire::Outcome::submitted;
    movement::wire::Receipt wire_bytes{};
    static_assert(sizeof(wire_bytes) == sizeof(pending->receipt));
    std::memcpy(&wire_bytes, &pending->receipt, sizeof(wire_bytes));
    constexpr char detail[] = "native_vendor_navigation_receipt_v3";
    if (!TryPublishResult(*runtime.storage, runtime.result_signal, static_cast<LONG64>(pending->sequence), pending->id,
        accepted ? ClientActionResultStage::submitted_to_client : ClientActionResultStage::rejected_by_client,
        accepted ? ERROR_SUCCESS : ERROR_REQUEST_ABORTED, detail, sizeof(detail) - 1, now,
        &wire_bytes, pending->execution_thread)) { return ERROR_NOT_ENOUGH_QUOTA; }
...
inline DWORD FinishGuardUpgradePending(Runtime& runtime, ULONGLONG now) noexcept {
    ...
    if (now > pending->deadline) {
        unsigned queued = 0;
        if (pending->state.compare_exchange_strong(queued, 1)) {
            guard_upgrade::wire::Receipt receipt{};
            receipt.host = pending->command.host; receipt.request = pending->command.request;
            receipt.window = pending->command.window;
            receipt.outcome = static_cast<unsigned>(guard_upgrade::wire::Outcome::stale);
            guard_upgrade::Complete(pending, receipt);
        }
    }
    if (pending->state.load(std::memory_order_acquire) != 2) { return ERROR_IO_PENDING; }
    const auto outcome = static_cast<guard_upgrade::wire::Outcome>(pending->receipt.outcome);
    const bool accepted = outcome == guard_upgrade::wire::Outcome::observed || outcome == guard_upgrade::wire::Outcome::submitted;
    movement::wire::Receipt wire_bytes{};
    static_assert(sizeof(wire_bytes) == sizeof(pending->receipt));
    std::memcpy(&wire_bytes, &pending->receipt, sizeof(wire_bytes));
    constexpr char detail[] = "native_guard_upgrade_receipt_v3";
    if (!TryPublishResult(*runtime.storage, runtime.result_signal, static_cast<LONG64>(pending->sequence), pending->id,
        accepted ? ClientActionResultStage::submitted_to_client : ClientActionResultStage::rejected_by_client,
        accepted ? ERROR_SUCCESS : ERROR_REQUEST_ABORTED, detail, sizeof(detail) - 1, now,
        &wire_bytes, pending->execution_thread)) { return ERROR_NOT_ENOUGH_QUOTA; }
    InterlockedExchange64(&runtime.storage->header.command_read_sequence, static_cast<LONG64>(pending->sequence));
    guard_upgrade::Release(pending); runtime.guard_upgrade_pending.reset(); return ERROR_SUCCESS;
}
inline DWORD FinishGuardFundingPending(Runtime& runtime, ULONGLONG now) noexcept {
    const auto& pending = runtime.guard_funding_pending;
    if (!pending) { return ERROR_SUCCESS; }
    // An expired request that never reached the owning thread cannot block the
    // channel forever while minimized/logged out. Never cancel an executing call.
    if (now > pending->deadline) {
        unsigned queued = 0;
        if (pending->state.compare_exchange_strong(queued, 1)) {
            guard_funding::wire::Receipt receipt{};
            receipt.host = pending->command.host; receipt.request = pending->command.request;
            receipt.window = pending->command.window;
            receipt.outcome = static_cast<unsigned>(guard_funding::wire::Outcome::stale);
            guard_funding::Complete(pending, receipt);
        }
    }
    if (pending->state.load(std::memory_order_acquire) != 2) { return ERROR_IO_PENDING; }
    const auto outcome = static_cast<guard_funding::wire::Outcome>(pending->receipt.outcome);
    const bool accepted = outcome == guard_funding::wire::Outcome::observed || outcome == guard_funding::wire::Outcome::submitted;
    movement::wire::Receipt wire_bytes{};
    static_assert(sizeof(wire_bytes) == sizeof(pending->receipt));
    std::memcpy(&wire_bytes, &pending->receipt, sizeof(wire_bytes));
    constexpr char detail[] = "native_guard_funding_receipt_v1";
    if (!TryPublishResult(*runtime.storage, runtime.result_signal, static_cast<LONG64>(pending->sequence), pending->id,
        accepted ? ClientActionResultStage::submitted_to_client : ClientActionResultStage::rejected_by_client,
        accepted ? ERROR_SUCCESS : ERROR_REQUEST_ABORTED, detail, sizeof(detail) - 1, now,
        &wire_bytes, pending->execution_thread)) { return ERROR_NOT_ENOUGH_QUOTA; }
    InterlockedExchange64(&runtime.storage->header.command_read_sequence, static_cast<LONG64>(pending->sequence));
    guard_funding::Release(pending); runtime.guard_funding_pending.reset(); return ERROR_SUCCESS;
}
...
inline DWORD DrainCommands(
    ...
    if (&storage == g_runtime.storage && g_runtime.vendor_pending) {
        const auto result = FinishVendorPending(g_runtime, now);
        if (result != ERROR_SUCCESS) { return result; }
    }
    if (&storage == g_runtime.storage && g_runtime.city_window_pending) {
        const auto result = FinishCityWindowPending(g_runtime, now);
        if (result != ERROR_SUCCESS) { return result; }
    }
    ...
    if (&storage == g_runtime.storage && g_runtime.guard_funding_pending) {
        const auto result = FinishGuardFundingPending(g_runtime, now);
        if (result != ERROR_SUCCESS) { return result; }
    }
    ...
    while (true) {
        ...
        if (snapshot.kind >= 17U && snapshot.kind <= 18U) {
            const auto verb = static_cast<guard_upgrade::wire::Verb>(snapshot.kind);
            guard_upgrade::wire::Command payload{};
            ...
            if (valid && &storage == g_runtime.storage && g_runtime.backing) {
                try {
                    ...
                    if (lease) {
                        ...
                        if (!guard_upgrade::Queue(command)) { return ERROR_RETRY; }
                        g_runtime.guard_upgrade_pending = std::move(command); return ERROR_IO_PENDING;
                    }
                } catch (...) { return ERROR_NOT_ENOUGH_MEMORY; }
            }
            constexpr char detail[] = "invalid_or_expired_guard_upgrade_lease";
        ...
        if (snapshot.kind >= 19U && snapshot.kind <= 21U) {
            ...
            const bool valid = snapshot.command_id && snapshot.payload_version == kClientActionPayloadVersion
                ...
                && movement::wire::Zero(snapshot.argument, sizeof(snapshot.argument))
                && movement::wire::Zero(snapshot.power_identifier, sizeof(snapshot.power_identifier))
                && guard_funding::wire::Valid(verb, payload);
            if (valid && &storage == g_runtime.storage && g_runtime.backing) {
                try {
                    auto lease = CaptureMovementLease(g_runtime.backing, payload.host, now);
                    if (lease) {
                        auto command = std::make_shared<guard_funding::QueuedCommand>();
                        command->id = snapshot.command_id; command->sequence = static_cast<std::uint64_t>(expected_sequence);
                        command->deadline = snapshot.deadline_tick; command->verb = verb;
                        command->command = payload; command->lease = std::move(lease);
                        if (!guard_funding::Queue(command)) { return ERROR_RETRY; }
                        g_runtime.guard_funding_pending = std::move(command); return ERROR_IO_PENDING;
                    }
                } catch (...) { return ERROR_NOT_ENOUGH_MEMORY; }
            }
            constexpr char detail[] = "invalid_or_expired_guard_funding_lease";
            if (!TryPublishResult(storage, result_signal, expected_sequence, snapshot.command_id,
                ClientActionResultStage::failed, ERROR_INVALID_DATA, detail, sizeof(detail) - 1, now)) {
                return ERROR_NOT_ENOUGH_QUOTA;
            }
        ...
        if ((snapshot.kind >= 13U && snapshot.kind <= 16U) || snapshot.kind == 22U) {
            ...
            const bool valid = snapshot.command_id && snapshot.payload_version == kClientActionPayloadVersion
                ...
                && movement::wire::Zero(snapshot.argument, sizeof(snapshot.argument))
                && movement::wire::Zero(snapshot.power_identifier, sizeof(snapshot.power_identifier))
                && vendor_navigation::wire::Valid(verb, payload);
            if (valid && &storage == g_runtime.storage && g_runtime.backing) {
                try {
                    auto lease = CaptureMovementLease(g_runtime.backing, payload.host, now);
                    if (lease) {
                        auto command = std::make_shared<vendor_navigation::QueuedCommand>();
                        command->id = snapshot.command_id; command->sequence = static_cast<std::uint64_t>(expected_sequence);
                        command->deadline = snapshot.deadline_tick; command->verb = verb;
                        command->command = payload; command->lease = std::move(lease);
                        if (!vendor_navigation::Queue(command)) { return ERROR_RETRY; }
                        g_runtime.vendor_navigation_pending = std::move(command); return ERROR_IO_PENDING;
                    }
                } catch (...) { return ERROR_NOT_ENOUGH_MEMORY; }
            }
        ...
        if (snapshot.kind >= 11U && snapshot.kind <= 12U) {
            const auto verb = static_cast<city_window::wire::Verb>(snapshot.kind);
            city_window::wire::Command payload{};
            std::memcpy(&payload, &snapshot.movement, sizeof(payload));
            const bool valid = snapshot.command_id && snapshot.payload_version == kClientActionPayloadVersion
                && snapshot.created_tick && snapshot.created_tick <= now && now <= snapshot.deadline_tick
                && snapshot.deadline_tick - snapshot.created_tick <= 5000
                ...
                && movement::wire::Zero(snapshot.argument, sizeof(snapshot.argument))
                && movement::wire::Zero(snapshot.power_identifier, sizeof(snapshot.power_identifier))
                && city_window::wire::Valid(verb, payload);
            if (valid && &storage == g_runtime.storage && g_runtime.backing) {
                try {
                    auto lease = CaptureMovementLease(g_runtime.backing, payload.host, now);
                    if (lease) {
                        auto command = std::make_shared<city_window::QueuedCommand>();
                        command->id = snapshot.command_id; command->sequence = static_cast<std::uint64_t>(expected_sequence);
                        command->deadline = snapshot.deadline_tick; command->verb = verb;
                        command->command = payload; command->lease = std::move(lease);
                        if (!city_window::Queue(command)) { return ERROR_RETRY; }
                        g_runtime.city_window_pending = std::move(command); return ERROR_IO_PENDING;
                    }
                } catch (...) { return ERROR_NOT_ENOUGH_MEMORY; }
            }
            constexpr char detail[] = "invalid_or_expired_city_window_lease";
            if (!TryPublishResult(storage, result_signal, expected_sequence, snapshot.command_id,
                ClientActionResultStage::failed, ERROR_INVALID_DATA, detail, sizeof(detail) - 1, now)) {
                return ERROR_NOT_ENOUGH_QUOTA;
            }
        ...
        if (snapshot.kind >= 8U && snapshot.kind <= 10U) {
            ...
            const bool valid = snapshot.command_id && snapshot.payload_version == kClientActionPayloadVersion
                ...
                && movement::wire::Zero(snapshot.argument, sizeof(snapshot.argument))
                && movement::wire::Zero(snapshot.power_identifier, sizeof(snapshot.power_identifier))
                && vendor::wire::Valid(verb, payload);
            if (valid && &storage == g_runtime.storage && g_runtime.backing) {
                try {
                    auto lease = CaptureMovementLease(g_runtime.backing, payload.host, now);
                    if (lease) {
                        auto command = std::make_shared<vendor::QueuedCommand>();
                        command->id = snapshot.command_id; command->sequence = static_cast<std::uint64_t>(expected_sequence);
                        command->deadline = snapshot.deadline_tick; command->verb = verb;
                        command->command = payload; command->lease = std::move(lease);
                        if (!vendor::Queue(command)) { return ERROR_RETRY; }
                        g_runtime.vendor_pending = std::move(command); return ERROR_IO_PENDING;
                    }
                } catch (...) { return ERROR_NOT_ENOUGH_MEMORY; }
            }
            constexpr char detail[] = "invalid_or_expired_vendor_lease";
            if (!TryPublishResult(storage, result_signal, expected_sequence, snapshot.command_id,
                ClientActionResultStage::failed, ERROR_INVALID_DATA, detail, sizeof(detail) - 1, now)) {
                return ERROR_NOT_ENOUGH_QUOTA;
            }
