...
if (-not (Test-Path -LiteralPath $pidPath -PathType Leaf)) {
    Write-Output "Shadowbane chat listener has no PID file."
    exit 0
}
...
if (-not [int]::TryParse($pidText, [ref] $listenerProcessId)) {
    throw "Shadowbane chat listener PID file is invalid: $pidText"
}

$process = Get-CimInstance Win32_Process -Filter "ProcessId = $listenerProcessId"
if ($null -eq $process) {
    Write-Output "Shadowbane chat listener is already stopped (former PID $listenerProcessId)."
    exit 0
}
if ($process.Name -ne "python.exe" -or
    $process.CommandLine -notmatch "shadowbane_lab\.cli\s+client\s+listen-go") {
    throw "PID $listenerProcessId does not belong to the Shadowbane chat listener."
}

Stop-Process -Id $listenerProcessId -Force
Write-Output "Shadowbane chat listener stopped (PID $listenerProcessId)."
