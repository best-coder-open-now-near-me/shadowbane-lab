...
function Stop-ExactManager {
    ...
    $familyIds = [Collections.Generic.HashSet[int]]::new()
    [void] $familyIds.Add($trackedId)
    do {
        $added = $false
        foreach ($candidate in $exactManagers) {
            ...
            if ($familyIds.Contains([int] $candidate.ProcessId)) {
                ...
                if ($parentIsExact -and $familyIds.Add([int] $candidate.ParentProcessId)) {
                    $added = $true
                }
            }
        }
    ...
    if ($familyProcesses.Count -ne $exactManagers.Count) {
        throw "Found competing manager process families; refusing to stop any of them."
    }
    ...
    Write-Output (
        "Stopped exact WonderBane manager process family before deployment: " +
