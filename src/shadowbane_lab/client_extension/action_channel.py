...
CLIENT_ACTION_CHANNEL_SCHEMA_VERSION = 3
...
class WindowsNativeActionCommandTransport:
    ...
    def renew_lease(self) -> None:
        ...
        started = self._kernel.tick_count()
        with self._lock, self._kernel.producer_lock(
            ...
            if heartbeat <= 0 or now < heartbeat or now - heartbeat > self._host_lease_timeout_ms:
                error = NativeActionChannelBusy("native action host lease expired")
                error.add_note(
                    f"heartbeat_age_ms={now - heartbeat}; renewal_wait_ms={now - started}"
                )
                raise error
            self._renew_host_lease()
