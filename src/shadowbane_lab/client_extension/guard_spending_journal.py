...
def _confirmed(command: Command, receipt: Receipt, operation: str) -> bool:
    ...
    # Native Capture revalidates ownership and selected roster membership.
    # UI addresses are disposable across a response; complete debit/progress
    # below, stable object keys and a frontmost owned guard page are required.
    owned_response = (
        before.eligible and not after.empty
        and a.front_hud == a.vendor_hud and b.front_hud == b.vendor_hud
        and b.mode == 6 and after.cost == before.cost
    )
    same = same_identity and (
        same_windows
        or (receipt.flags & upgrade.REOPENED and b.front_hud == b.vendor_hud)
        or (not receipt.flags & upgrade.REOPENED and owned_response)
    )
    return bool(
        same
