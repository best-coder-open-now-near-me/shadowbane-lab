"""Typed building/hireling navigation correlated with the exact client and producer lease."""
...
class NativeVendorNavigationCommand:
    command_id: int
    kind: Verb
    payload: Command
    ...
    def encode_slot(self, *, sequence: int, created_tick: int, deadline_tick: int) -> bytes:
        if type(self.command_id) is not int or not 0 < self.command_id < 2**64:
            raise ValueError("vendor_navigation command ID must be a positive uint64")
        if (
            not 0 < sequence < 2**63
            or not 0 < created_tick <= deadline_tick < 2**64
            or deadline_tick - created_tick > 5000
        ):
            raise ValueError("invalid vendor_navigation command sequence/deadline")
...
class NativeVendorNavigationSession:
    ...
    def _submit(
        ...
        if self._closed:
            raise channel.NativeActionChannelUnavailable("vendor_navigation session is closed")
    ...
    def open_building(self, expected: Snapshot, building_id: int, request_key: str) -> Receipt:
        return self._submit(Verb.BUILDING, request_key, expected, building_id)

    def open_vendor(
        self, expected: Snapshot, building_id: int, vendor_id: int, request_key: str
    ) -> Receipt:
