"""Typed guard_upgrade commands, correlated with the exact client and producer lease."""
...
class NativeGuardUpgradeCommand:
    command_id: int
    kind: Verb
    payload: Command
    ...
    def encode_slot(self, *, sequence: int, created_tick: int, deadline_tick: int) -> bytes:
        if type(self.command_id) is not int or not 0 < self.command_id < 2**64:
            raise ValueError("guard_upgrade command ID must be a positive uint64")
        if (
            not 0 < sequence < 2**63
            or not 0 < created_tick <= deadline_tick < 2**64
            or deadline_tick - created_tick > 5000
        ):
            raise ValueError("invalid guard_upgrade command sequence/deadline")
...
class _RetryableInspectionError(channel.NativeActionChannelUnavailable):
    """A rejected read-only observation; never used for Upgrade."""
...
class NativeGuardUpgradeSession:
    ...
    def _submit(self, verb: Verb, request_key: str, expected: Snapshot | None = None) -> Receipt:
        if self._closed:
            raise channel.NativeActionChannelUnavailable("guard_upgrade session is closed")
        ...
        def dispatch() -> Receipt:
            ...
            if result.detail != "native_guard_upgrade_receipt_v3":
                error = (
    ...
    def upgrade(self, expected: Snapshot, request_key: str) -> Receipt:
        return self._submit(Verb.UPGRADE, request_key, expected)
