...
def compile_texture_profile(
    ...
    *,
    ...
    providers: defaultdict[tuple[int, int], list[TextureProvider]] = defaultdict(list)
    cache_relative_paths: set[str] = set()
    try:
        ...
        if len(cache_relative_paths) != 1:
            raise TextureProfileError(
                "selected texture components target different cache paths"
            )
        cache_relative_path = next(iter(cache_relative_paths))
        _require_source_path_match(source_cache, cache_relative_path)
        selected = _resolve_providers(providers, resolutions or {})
        artifacts = {item.key: Path(item.artifact_path) for item in selected}
        ...
        raise
    except (OSError, TextureCacheError, TexturePatchError, TypeError, ValueError) as exc:
        raise TextureProfileError(f"could not compile texture profile: {exc}") from exc
    if sha256_file(source_cache) != source_digest:
        raise TextureProfileError("source cache changed while the profile was compiled")
...
def _collect_package_providers(
    ...
    for component in package.manifest.components:
        ...
        cache_relative_paths.add(patch_manifest.cache_relative_path)
        if patch_manifest.source_cache_sha256 != source_digest:
            raise TextureProfileError(
                f"{package.identity}:{component.component_id} targets a different source cache"
            )
        for replacement in patch_manifest.replacements:
            artifact_path = artifact_root / replacement.artifact_file_name
            if not artifact_path.is_file() or is_reparse_point(artifact_path):
                raise TextureProfileError(
                    f"texture artifact is not an ordinary file: {artifact_path}"
                )
        ...
        provider_id = f"{package.manifest.mod_id}:{component.component_id}"
...
def _resolve_providers(
    providers: Mapping[tuple[int, int], Sequence[TextureProvider]],
    ...
    for key, provider_id in resolutions.items():
        ...
        validate_identifier(provider_id, "texture conflict provider_id")
    ...
    for key in sorted(providers):
        ...
        signatures = {item.result_signature for item in candidates}
        if len(signatures) == 1:
            selected.append(candidates[0])
            continue
        requested = normalized_resolutions.get(key)
        if requested is None:
            conflicts.append(TextureConflict(key[0], key[1], candidates))
            continue
        matches = tuple(item for item in candidates if item.provider_id == requested)
        if len(matches) != 1:
            raise TextureProfileError(
                f"resolution for {key[0]}:{key[1]} does not name exactly one provider"
            )
        selected.append(matches[0])
        used_resolutions.add(key)
    if conflicts:
        raise TextureProfileConflictError(conflicts)
    unused = sorted(normalized_resolutions.keys() - used_resolutions)
    if unused:
        text = ", ".join(f"{group}:{resource}" for group, resource in unused)
        raise TextureProfileError(f"texture conflict resolutions were not used: {text}")
    if not selected:
        raise TextureProfileError("selected packages provide no texture resources")
...
def _cross_check_cache_plan(
    ...
    for write in cache_plan.writes:
        ...
        if (
            write.source_payload_sha256 != provider.source_payload_sha256
            or write.result_payload_sha256 != provider.result_payload_sha256
            ...
            raise TextureProfileError(
                f"combined cache plan differs for {write.group_id}:{write.resource_id}"
            )


def _require_source_path_match(source_cache: Path, relative_path: str) -> None:
    parts = PurePosixPath(relative_path).parts
    if len(source_cache.parts) < len(parts):
        raise TextureProfileError(
            "source cache path does not match the compiled cache-relative path"
        )
    actual = tuple(part.casefold() for part in source_cache.parts[-len(parts) :])
