...
class TextureProfileConflictError(TextureProfileError):
    ...
    def __init__(self, conflicts: Sequence[TextureConflict]) -> None:
        ...
        if not self.conflicts:
            raise ValueError("texture conflict errors require at least one conflict")
        summary = ", ".join(
            f"{item.group_id}:{item.resource_id}" for item in self.conflicts
        )
        super().__init__(f"texture profile has unresolved conflicts: {summary}")
...
class TextureProvider:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("texture dimensions or channel count are unsupported")
...
class TextureConflict:
    ...
    def __post_init__(self) -> None:
        if len(self.providers) < 2:
            raise ValueError("texture conflicts require at least two providers")
        if any(not isinstance(item, TextureProvider) for item in self.providers):
            raise ValueError("texture conflicts contain an unsupported provider")
        expected = (self.group_id, self.resource_id)
        if any(item.key != expected for item in self.providers):
            raise ValueError("texture conflict providers target different resources")
...
class TextureProfilePlan:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != TEXTURE_PROFILE_SCHEMA_VERSION:
            raise ValueError("unsupported texture-profile schema version")
        ...
        if not self.packages or not self.selected:
            raise ValueError("texture profiles require packages and selected resources")
        if any(not isinstance(item, AssetModPackage) for item in self.packages):
            raise ValueError("texture-profile packages contain an unsupported value")
        if any(not isinstance(item, TextureProvider) for item in self.selected):
            raise ValueError("selected texture resources contain an unsupported value")
        if not isinstance(self.cache_plan, TextureCachePlan):
            raise ValueError("cache_plan must be a TextureCachePlan")
        if tuple(sorted(self.packages, key=lambda item: item.identity)) != self.packages:
            raise ValueError("texture-profile packages must use identity order")
        if tuple(sorted(self.selected, key=lambda item: item.key)) != self.selected:
            raise ValueError("selected texture resources must use resource-key order")
        if len({item.key for item in self.selected}) != len(self.selected):
            raise ValueError("selected texture resources contain duplicate keys")
        if self.cache_plan.source_cache_sha256 != self.source_cache_sha256:
            raise ValueError("cache plan source digest differs from the texture profile")
        if self.cache_plan.targeted_keys != self.targeted_keys:
            raise ValueError("cache plan targets differ from the selected providers")
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "selected_resources": [item.as_dict() for item in self.selected],
        }
