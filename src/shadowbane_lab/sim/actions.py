...
class AttackModifierSpec:
    """Modify a qualifying weapon attack before its hit and damage rolls."""
...
class WeaponAttackSpec:
    ...
    def __post_init__(self) -> None:
        ...
        _identifier(self.attack_rating_scalar, "attack_rating_scalar")
        _identifier(self.defense_scalar, "defense_scalar")
        for value, name in (
...
class ActionTriggerSpec:
    """Payload and optional attack modifier armed for a later action."""

    trigger_key: str
    payload: tuple[DirectEffectPrimitive, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(effect, _DIRECT_EFFECT_TYPES) for effect in self.payload):
            raise ValueError("action trigger payload must contain direct effect primitives")
        for values, name in (
